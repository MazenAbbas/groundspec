...
The Task Contract schema has gone through this exactly once so far: `task_contract.v0_2_0.schema.json` was added alongside the untouched `task_contract.v0_1_0.schema.json` to add `high_value` to `scope.open_questions[].classification`'s enum -- needed because the Meta-Skill's clarification policy (`references/clarification-policy.md`) distinguishes a fourth category (worth asking if budget allows, but no safe default exists) that the original three-value enum (`blocking`/`important_defaultable`/`optional`) couldn't represent. This is a purely additive change: every field, every other enum, and every `$defs` entry is byte-identical between the two schema files except that one enum's value list (verified by `tests/unit/test_schema_versioning.py`). No migration function was needed because no existing document ever used a value the change removes or renames -- a `0.1.0` document keeps validating against the untouched `0.1.0` schema file forever, and `groundspec` ships and supports both versions simultaneously (`SUPPORTED_CONTRACT_VERSIONS = ("0.1.0", "0.2.0")`). New contracts (`groundspec create`, or `contract.factory.new_contract`) default to `0.2.0`.
