...
- Never let a critical residual risk or a weak-evidence 'must' criterion get waved through to `PASS` -- `references/execution-and-verification.md` defines exactly which conditions force `FAIL`/`INCOMPLETE` instead, and omitting a field to dodge a gate does not work (defaults are conservative by design).
