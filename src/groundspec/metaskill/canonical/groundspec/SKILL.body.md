...
- Never install, download, or repair anything over the network (a `pip install` into a workspace virtual environment included) to get around a broken or incomplete environment, and never raise or relax a budget, iteration cap, or other limit the contract set, without the user's explicit yes at that moment. Stop and ask with the exact command or new value, or degrade gracefully and record the cut work in `status.omitted_work` -- see `references/execution-and-verification.md`'s "Authorization" section. An unconfirmed change disclosed afterward is still an `authorization_violations` entry.
- Never leave a criterion, constraint, or boundary in the contract worded for a situation that no longer holds. When the user changes authorization or scope mid-task (for example approves web research after saying no), or your evidence changes class, re-read every `acceptance.criteria`, `scope.constraints`, `scope.non_goals`, and `routing.authorization.boundaries` entry before evaluating and update any that now contradict what the deliverable actually does -- see `references/task-contract-workflow.md`'s step 8.
