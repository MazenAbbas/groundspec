...
- `acceptance_criteria_results`: for each `acceptance.criteria` entry (at minimum, every `must`-priority one), either a plain `true`/`false` (legacy shape, still supported), or -- preferred -- `{"met": bool, "evidence_label": "<label>"}` so the evaluator can check the label matches the strength the criterion's own `verification_method` actually requires (see "Evidence taxonomy" below). Missing a `must` criterion here produces `INCOMPLETE`. **Record `false` only for a check that actually ran and failed.** If a check could not run at all (a package you were not allowed to install, an approval you never got, a service you cannot reach), leave that criterion out of `acceptance_criteria_results` and say why in `status.omitted_work`: the honest state is `INCOMPLETE` ("not enough evidence to say"), not `FAIL` ("checked and wrong"). Writing `false` for something you never verified overstates what you know and turns a blocked task into a failed one.
...
- installing or downloading anything: `pip install`, `npm install`, `apt`/`brew`/`winget`, `curl`/`wget`, a model or dataset download. A package install reaches the network and changes the machine, so it is both a network action and a filesystem mutation, however routine it feels;
- raising, relaxing, or otherwise editing a limit the user or the contract set: `budget.tool_call_budget`, `time_budget_minutes`, iteration caps, `max_clarification_questions`, `routing.authorization` entries. A limit is a boundary; you do not get to move it because you are running short;
...
**A broken or incomplete environment is not a license to fix it yourself.** If a needed package is missing, an interpreter or `pip` is broken, or a tool you planned to use cannot run, stop before working around it over the network. Your options, in order: (1) use what is already available, e.g. the standard library or a tool already installed; (2) ask one concise question naming the exact command you would run (for example the specific `pip install` and target directory) and wait for a clear yes; (3) if no approval is available, finish the parts that do not need it, record the rest in `status.omitted_work`, and report `INCOMPLETE` honestly. Installing into a virtual environment inside the workspace is still a network download and still needs the yes. If you already did it without asking, it is an `authorization_violations` entry in `evidence.json`, disclosed plainly. Doing it quietly and describing it only in prose is the failure this rule exists to prevent.

**Never raise, relax, or edit a budget or limit on your own.** If you are about to run out of `tool_call_budget`, `time_budget_minutes`, or iterations, do not edit the contract to give yourself more. Tell the user which limit is close, what remains, and what an extension would buy, then either wait for their answer or degrade gracefully: finish the highest-value remaining work inside the existing limit, list everything cut in `status.omitted_work`, and let the completion state reflect that. If the user does approve an extension, record their approval (their words) in the contract next to the changed value. A limit you changed silently and disclosed afterward is still an unconfirmed change and belongs in `authorization_violations`.
