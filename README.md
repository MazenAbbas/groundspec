...
A contract is one JSON or TOML document validated against a versioned JSON Schema (currently [`0.1.0`](src/groundspec/schema/task_contract.v0_1_0.schema.json), [`0.2.0`](src/groundspec/schema/task_contract.v0_2_0.schema.json), [`0.3.0`](src/groundspec/schema/task_contract.v0_3_0.schema.json), or [`0.4.0`](src/groundspec/schema/task_contract.v0_4_0.schema.json) -- all four fully supported; see [docs/schema-reference.md](docs/schema-reference.md) for exactly what changed at each step). Top-level sections: `brief` (raw request, normalized problem, goal, users, deliverables), `scope` (constraints, non-goals, assumptions, open questions), `routing` (domain packs, risk overlays, risk level, authorization), `quality` (hard constraints, soft objectives), `acceptance` (criteria with a verification method and required evidence per item), `budget` (time/iteration/tool/cost limits, plus a reserved verification fraction), `control` (stop/escalation conditions), and `status` (completion state, verified facts, unverified claims, remaining uncertainty, residual risks). Full field-by-field reference: [docs/schema-reference.md](docs/schema-reference.md).
...
- **Meta-Skill, thirteen live runs total:** [examples/metaskill/03-software-csv-export](examples/metaskill/03-software-csv-export/) (`v0.2.0rc1`); four independent forward tests in `v0.2.0rc2` ([docs/forward-tests-v0.2.0rc2.md](docs/forward-tests-v0.2.0rc2.md)); and eight more in the local `0.2.0rc3` candidate ([docs/forward-tests-v0.2.0rc3.md](docs/forward-tests-v0.2.0rc3.md)): a food-delivery PRD scenario with research authorized and, separately, with all external actions prohibited; a small unambiguous software task; a regulated scenario with only secondary legal sources available; a scenario with a missing material geographic decision; a bike-share PRD run directly by the engineer (not delegated) specifically to prove the gates engage outside of any prepared fixture; a content-domain marketing task touching customer personal data; and a destructive/irreversible data-deletion task combining four previously-unexercised risk overlays at once. Together these runs exercise all 3 domain packs and all 7 risk overlays this project defines -- see the coverage matrix in [docs/forward-tests-v0.2.0rc3.md](docs/forward-tests-v0.2.0rc3.md). Each was independently re-verified against its actual produced files afterward (including re-running an automated test suite personally, not just reading its reported output) -- not accepted on the agent's word. Together the first five `v0.2.0rc2` runs found and led to fixing nine real gaps; the eight `v0.2.0rc3` runs confirmed all seven defects from that release's follow-up regression report fixed and found no new product gaps (two self-disclosed testing-harness incidents -- a stray file write outside the assigned workspace, both times from the same working-directory-reset cause -- were independently confirmed harmless, not Groundspec defects). Thirteen runs across three releases is still not a statistically powered evaluation.
- **Meta-Skill, two constructed examples:** [examples/metaskill/](examples/metaskill/)'s remaining two scenarios demonstrate the target contract shape but were authored during development, not captured from a live run -- each says so in its own `session-notes.md`.
