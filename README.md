...
- **Meta-Skill, five live runs total:** [examples/metaskill/03-software-csv-export](examples/metaskill/03-software-csv-export/) (`v0.2.0rc1`) plus four independent forward tests in `v0.2.0rc2` ([docs/forward-tests-v0.2.0rc2.md](docs/forward-tests-v0.2.0rc2.md)): the same food-delivery PRD scenario with external actions prohibited and, separately, with read-only research explicitly permitted; a small unambiguous software task; and an audit task containing an embedded prompt-injection attempt. Each was a fresh, isolated subagent following only the exported Skill files and the real CLI, with every claim independently re-verified against its actual produced files afterward -- not accepted on the agent's word. Together these five runs found and led to fixing nine real gaps across two releases, including the `v0.2.0rc1` defect report itself. Five runs across two releases is still not a statistically powered evaluation.
