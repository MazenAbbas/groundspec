...
- **Meta-Skill, ten live runs total:** [examples/metaskill/03-software-csv-export](examples/metaskill/03-software-csv-export/) (`v0.2.0rc1`); four independent forward tests in `v0.2.0rc2` ([docs/forward-tests-v0.2.0rc2.md](docs/forward-tests-v0.2.0rc2.md)); and five more in the local `0.2.0rc3.dev0` candidate ([docs/forward-tests-v0.2.0rc3.md](docs/forward-tests-v0.2.0rc3.md)): a food-delivery PRD scenario with research authorized and, separately, with all external actions prohibited; a small unambiguous software task; a regulated scenario with only secondary legal sources available; and a scenario with a missing material geographic decision. Each was a fresh, isolated subagent following only the exported Skill files and the real CLI, with every claim independently re-verified against its actual produced files afterward -- not accepted on the agent's word. Together the first five runs found and led to fixing nine real gaps across two releases; the second five runs confirmed all seven defects from the `v0.2.0rc2` follow-up regression report fixed and found no new product gaps (one self-disclosed testing-harness incident -- a stray file write outside the assigned workspace -- was independently confirmed harmless, not a Groundspec defect). Ten runs across three releases is still not a statistically powered evaluation.
...
- **The Meta-Skill has ten genuine live-run evaluations and two constructed examples**, not a full evaluation suite -- see [Current evaluation status](#current-evaluation-status).
